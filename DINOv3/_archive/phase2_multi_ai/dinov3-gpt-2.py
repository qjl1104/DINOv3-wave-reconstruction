...
- 自动对齐 tile/overlap 至 patch 的倍数；一次性打印
- 成对一致的数据增强（亮度/对比度/噪声）
- AMP 新 API（torch.amp）
- 可选 TensorBoard（未安装也能跑）
- 分目录保存（避免与旧实验串味）
- 训练/验证可视化，历史曲线保存
- 视差上限命中率监控（提示是否需增大 MAX_DISPARITY）

需求：
pip install torch torchvision transformers opencv-python matplotlib tqdm tensorboard (可选)
"""
...
class Config:
    ...
    MAX_DISPARITY: int = 160   # 建议 160（或 144），避免上限截断
...
class ImprovedSelfSupervisedLoss(nn.Module):
    ...
    def inverse_warp(self, features, disp):
        ...
        pixel_coords = torch.stack([x_coords, y_coords], dim=0).float().repeat(B, 1, 1, 1)  # [B,2,H,W]
        disp = disp.squeeze(1)
...
class DINOv3StereoModel(nn.Module):
    ...
    def forward(self, left_image, right_image):
        ...
        cost_volume = self.build_cost_volume(left_feat, right_feat)           # [B,C,D,Hf,Wf]
        ...
        # 简单的“上限命中率”监控（像素尺度）
        cap_threshold = (self.max_disp - self.patch_size)  # 接近最大可回归视差的阈值
        with torch.no_grad():
...
class Trainer:
    ...
    def train(self):
        ...
        for epoch in range(self.cfg.NUM_EPOCHS):
            ...
            print(f"Epoch {epoch + 1}/{self.cfg.NUM_EPOCHS} -> 验证损失: {avg_val_loss:.4f}, PSNR: {val_metrics.get('psnr', 0):.2f}, cap: {val_metrics.get('cap_ratio', 0):.3f}")
