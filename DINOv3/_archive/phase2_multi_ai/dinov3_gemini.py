...
class GroundTruthGenerator:
    ...
    def match_and_triangulate(self, left_markers: np.ndarray, right_markers: np.ndarray) -> np.ndarray:
        if left_markers.size == 0 or right_markers.size == 0:
            return np.array([])
    ...
    def process_all_frames(self):
        ...
        for i, l_path in enumerate(tqdm(left_image_paths, desc="处理帧")):
            ...
            if i % 100 == 0:
                print(f"\n处理第 {i} 帧: {basename}")
                print(f"左图检测到 {len(left_markers)} 个标记点")
                print(f"右图检测到 {len(right_markers)} 个标记点")
            ...
            # 添加调试信息
            if i % 100 == 0:
                print(f"成功匹配 {len(points_3d) if points_3d is not None else 0} 个点")
