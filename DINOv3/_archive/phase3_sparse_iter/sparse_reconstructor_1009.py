...
class Config:
    ...
    SMOOTHNESS_WEIGHT: float = 0.01  # Sparse smoothness weight
...
def auto_tune_config(cfg: Config):
    ...
    if free_mem_mb < 8000:
        ...
        cfg.BATCH_SIZE, cfg.MAX_KEYPOINTS, cfg.GRADIENT_ACCUMULATION_STEPS, scale = 2, 512, 1, 1.0
...
class SparseKeypointDetector(nn.Module):
    ...
    def non_maximum_suppression(self, keypoints, scores):
        ...
        kp_sorted = keypoints[sorted_indices]
        scores_sorted = scores[sorted_indices]

        keep_mask = torch.ones(len(kp_sorted), dtype=torch.bool, device=keypoints.device)
        ...
        return kp_sorted[keep_mask], scores_sorted[keep_mask]
...
class SparseMatchingNetwork(nn.Module):
    ...
    def forward(self, desc_left, desc_right, kp_left, kp_right, image_size):
        ...
        x_left = kp_left[:, :, 0]  # (B, N_l)
        x_right = kp_right[:, :, 0]  # (B, N_r)

        # Disparity = x_left - weighted_x_right
        disparity = x_left.unsqueeze(2) - x_right.unsqueeze(1)  # (B, N_l, N_r)
        disparity_pred = (disparity * match_probs).sum(dim=2)  # (B, N_l)
...
class SparsePhotometricLoss(nn.Module):
    """Photometric loss on sparse keypoint patches"""
    ...
    def forward(self, left_img, right_img, keypoints_left, disparity, scores_left):
        """
        Args:
            left_img: (B, 1, H, W) - grayscale left image
            right_img: (B, 1, H, W) - grayscale right image
            keypoints_left: (B, N, 2) - left keypoints
            disparity: (B, N) - predicted disparity
            scores_left: (B, N) - keypoint scores (for weighting)
        Returns:
            photo_loss: scalar
            smooth_loss: scalar
        """
        B, N, _ = keypoints_left.shape
        ...
        for b in range(B):
            for i in range(N):
                if scores_left[b, i] < 0.1:  # Skip padded keypoints
                    continue

                x_left = keypoints_left[b, i, 0]
                y_left = keypoints_left[b, i, 1]
                disp = disparity[b, i]

                x_right = x_left - disp

                # Check if patches are within bounds
                if not self._is_valid_patch(x_left, y_left, H, W):
                    continue
                if not self._is_valid_patch(x_right, y_left, H, W):
                    continue

                # Extract patches
                patch_left = self._extract_patch(left_img[b, 0], x_left, y_left)
                patch_right = self._extract_patch(right_img[b, 0], x_right, y_left)

                # L1 loss
                photo_loss += F.l1_loss(patch_left, patch_right)
                valid_count += 1

        if valid_count > 0:
            photo_loss /= valid_count

        # Sparse smoothness: nearby keypoints should have similar disparity
        smooth_loss = self._compute_sparse_smoothness(keypoints_left, disparity, scores_left)

        return photo_loss, smooth_loss

    def _is_valid_patch(self, x, y, H, W):
        """Check if patch centered at (x, y) is within image bounds"""
        hp = self.half_patch
        x_val = float(x) if isinstance(x, torch.Tensor) else x
        y_val = float(y) if isinstance(y, torch.Tensor) else y
        return (x_val >= hp and x_val < W - hp and y_val >= hp and y_val < H - hp)
    ...
    def _extract_patch(self, image, x, y):
        ...
        patch = image[y_int - hp:y_int + hp + 1, x_int - hp:x_int + hp + 1]
        return patch
...
class Trainer:
    ...
    def visualize(self, data, outputs, step, phase):
        ...
        if len(disp_valid) > 0:
            axes[1].hist(disp_valid, bins=50, alpha=0.7, edgecolor='black')
