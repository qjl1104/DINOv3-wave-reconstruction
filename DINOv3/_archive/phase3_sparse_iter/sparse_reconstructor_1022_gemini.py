...
class FeatureMetricLoss(nn.Module):
    ...
    def forward(self, desc_left, desc_right, match_scores, keypoints_left, disparity, scores_left,
                constraint_mask):  # Added constraint_mask
        """
        Args:
...
class Trainer:
    ...
    def plot_training_history(self):
        ...
        if self.history['train']['total'] and self.history['val']['total']:
            all_losses = [l for l in self.history['train']['total'] + self.history['val']['total'] if np.isfinite(l)]
            if all_losses:
                min_loss = min(all_losses)
                axes[0, 0].set_ylim(bottom=min_loss - 0.01)
        ...
        # --- MODIFICATION: Update loss component plot ---
        axes[1, 1].plot(self.history['train']['feature'], label='Feature Loss')
        ...
        try:
            with open(self.log_file, 'w') as f:
                json.dump(log_data, f, indent=2,
                          default=lambda o: str(o) if isinstance(o, (np.float32, np.float64)) else o)
        except Exception as e:
