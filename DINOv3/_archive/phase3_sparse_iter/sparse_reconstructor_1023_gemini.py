...
class Config:
    ...
    PHOTOMETRIC_WEIGHT: float = 1.0  # Weight for the new FEATURE loss

    # --- v2.3 FUNDAMENTAL MODIFICATION: Reduce smoothness weight ---
    # The unweighted smoothness loss (~300) was dominating the feature loss (~0.23).
    # This change forces the optimizer to prioritize feature matching.
...
class SparseMatchingNetwork(nn.Module):
    ...
    def forward(self, desc_left, desc_right, kp_left, kp_right, image_size):
        ...
        # --- MODIFICATION: Add epsilon to F.normalize for numerical stability ---
        eps = 1e-8
        ...
        # Return the *unconstrained* scores for the loss function,
        # but the *constrained* disparity prediction.
        # The loss function will re-apply constraints as needed.
        # ** Correction: Return the constrained scores. The loss needs to know which matches
        # were considered invalid.
        # --- v2.2 MODIFICATION: Pass constraint_mask to loss ---
...
class FeatureMetricLoss(nn.Module):
    ...
    This is robust to non-Lambertian surfaces (like specular highlights)
    and sparse data.
    """
    ...
    def forward(self, desc_left, desc_right, match_scores, keypoints_left, disparity, scores_left, constraint_mask): # Added constraint_mask
        """
        Args:
            desc_left: (B, N_l, C) Left descriptors
            desc_right: (B, N_r, C) Right descriptors
        ...
        # 1. Feature Loss (replaces Photometric)

        # Use the same temperature as the matcher network for consistency
        ...
        # --- v2.2 MODIFICATION: Fix NaN from all-invalid rows ---
        # F.softmax([-inf, -inf]) -> [NaN, NaN]. We must convert these to 0.
        match_probs = torch.nan_to_num(match_probs, nan=0.0)
        # --- END MODIFICATION ---

        # Get weighted average of right descriptors based on match probabilities
        # (B, N_l, N_r) bmm (B, N_r, C) -> (B, N_l, C)
        desc_right_weighted = torch.bmm(match_probs, desc_right)

        # --- MODIFICATION: Add epsilon to F.normalize for numerical stability ---
        # This prevents NaN gradients if a vector is all zeros.
        eps = 1e-8 # Use a small fixed epsilon
        ...
        # We want to *maximize* similarity, so we *minimize* (1 - sim)
        feature_loss_per_kp = 1.0 - cosine_sim # (B, N_l)

        # --- v2.2 MODIFICATION: Use a more robust mask ---
        # Mask out invalid (padded) keypoints
        detection_mask = (scores_left > 0.1) # (B, N_l)
        # Mask out keypoints that had NO valid match candidates
        ...
        # 2. Smoothness Loss (unchanged logic)
    ...
    def _compute_sparse_smoothness(self, keypoints, disparity, scores):
        ...
        # Return the raw, unweighted smoothness loss
        return smooth_loss
...
class SparseMatchingStereoModel(nn.Module):
    ...
    def forward(self, left_gray, right_gray, left_rgb, right_rgb, mask):
        ...
        # 3. Match keypoints
        # --- v2.2 MODIFICATION: Catch constraint_mask ---
        ...
        return {
            ...
            'disparity': disparity,
            'constraint_mask': constraint_mask # Pass mask to trainer
        }
        # --- END MODIFICATION ---
...
class EvaluationMetrics:
    ...
    def compute_sparse_metrics(disparity, keypoints_left, scores_left):
        ...
        if valid_mask.sum() > 0:
            ...
            # --- Robustness: Filter NaNs before mean/std ---
...
class Trainer:
    def __init__(self, cfg: Config):
        ...
        # --- MODIFICATION: Use new loss function ---
    ...
    def _run_epoch(self, epoch, is_training):
        ...
        for data in pbar:
            ...
            with torch.amp.autocast('cuda', enabled=self.cfg.USE_MIXED_PRECISION):
                ...
                # --- MODIFICATION: Call new loss function ---
                # --- v2.2 MODIFICATION: Pass constraint_mask to loss ---
                feature_loss, smooth_loss = self.loss_fn(
                    ...
                    outputs['disparity'],
                    outputs['scores_left'],
                    outputs['constraint_mask'] # Pass new mask
                )
            ...
            # Accumulate results
            # --- MODIFICATION: Update result keys ---
            ...
            epoch_results['smoothness'] += smooth_loss.item() if torch.isfinite(smooth_loss) else 0.0 # Store unweighted smooth loss
            # --- End Modification ---
    ...
    def _log_epoch_results(self, phase, epoch, results):
        for k, v in results.items():
            ...
            if self.writer:
                ...
                # Log unweighted smoothness for better scale comparability if needed elsewhere
                if k == 'smoothness':
    ...
    def visualize(self, data, outputs, step, phase):
        ...
        if len(disp_valid) > 0 and np.all(np.isfinite(disp_valid)):
            ...
            axes[1].grid(True, alpha=0.3)
             ...
             axes[1].set_title('Disparity Distribution (No valid keypoints)')
    ...
    def plot_training_history(self):
        ...
        # Adjust Y-axis limits for better visualization if data exists
        ...
        # Calculate weighted smoothness loss for plotting its contribution
        weighted_smooth_train = [s * self.cfg.SMOOTHNESS_WEIGHT for s in self.history['train']['smoothness']]
        feature_train = self.history['train']['feature'] # Already weighted by PHOTOMETRIC_WEIGHT=1.0
    ...
    def update_log_file(self, epoch):
        ...
        try:
            with open(self.log_file, 'w') as f:
                # Use a custom handler for numpy floats
                json.dump(log_data, f, indent=2, default=lambda o: float(o) if isinstance(o, (np.float32, np.float64)) else str(o) if isinstance(o, np.ndarray) else o)
        except Exception as e:
