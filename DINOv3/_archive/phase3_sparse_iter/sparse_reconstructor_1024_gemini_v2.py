...
#
# v2 Change:
# Replaced SparsePhotometricLoss (pixel-based) with FeatureMetricLoss (feature-based).
# The loss is now computed on the cosine similarity between DINO descriptors,
# which is more robust to sparse, non-Lambertian data.
#
# v2.1 (NaN Fix):
# Added epsilon to F.normalize in FeatureMetricLoss to prevent NaN gradients
# from zero-vectors.
#
# v2.2 (NaN Fix 2):
# Added torch.nan_to_num to softmax and robust masking in loss
# to handle cases where no valid matches exist for a keypoint.
#
# v2.3 (Fundamental Fix):
# Drastically reduced SMOOTHNESS_WEIGHT. The unweighted smoothness loss
# was ~300, while feat_loss was ~0.23. The smoothness term was
# completely dominating the optimization. This change makes
# feature matching the primary optimization goal.
#
# v2.4 (Plot Fix):
# Restored missing plots for Keypoints and Mean Disparity in training history.
# Corrected Loss Components plot to show weighted smoothness loss contribution.
#
# v2.5 (Fine-tuning Attempt):
# Reduced LEARNING_RATE to 1e-5 previously. Now reverted for model complexity change.
#
# v2.6 (Increase Complexity):
# Increased NUM_ATTENTION_LAYERS from 4 to 6.
# Reverted LEARNING_RATE to 1e-4.
#
# v2.7 (Adjust Smoothness):
# Moderately increased SMOOTHNESS_WEIGHT from 0.0001 to 0.001
# to potentially reduce noise/flickering in disparity map.
#
# v2.8 (Manual ROI Left):
# Hardcoded the manually selected roi_left values in the Dataset class previously.
#
# v2.9 (Manual ROI Right):
# Hardcoded the manually selected roi_right values in the Dataset class previously.
# Adjusted Resolution to match ROI Left aspect ratio previously.
#
# v2.10 (Unified ROI):
# Hardcoded unified, vertically aligned ROIs based on manual selections.
# Adjusted Resolution to 896x416 based on ROI aspect ratio.
#
# v2.11 (Fix Matplotlib Backend):
# Set matplotlib backend to 'Agg' to prevent Tkinter/Tcl runtime errors.
...
# --- v2.11 MODIFICATION: Set Matplotlib Backend ---
import matplotlib
...
class Config:
    ...
    # --- Resolution Setting (v2.10 Adjusted for Unified ROI ratio ~1.98 for Left, ~1.86 for Right) ---
    # Using 896x416 (ratio 2.15) as a compromise close to ROI ratios
    IMAGE_HEIGHT: int = 416  # Matches ratio ~2.15 -> 896x416
    IMAGE_WIDTH: int = 896
    ...
    # PATCH_SIZE is no longer used by the new FeatureMetricLoss, but kept for config compatibility
    ...
    # Increase if using higher resolution causes OOM errors
    GRADIENT_ACCUMULATION_STEPS: int = 2  # Adjusted for 896x416 resolution
...
def auto_tune_config(cfg: Config):
    """ DEPRECATED - We are manually setting resolution now. """
    print("--- auto_tune_config: SKIPPED (Manual resolution set in Config) ---")
    # Keep the function definition but don't execute logic if called
    return cfg


# --- 1. Sparse Keypoint Detector ---
class SparseKeypointDetector(nn.Module):
    """Detect sparse keypoints (bright specular points) on grayscale images using NMS."""
    ...
    def forward(self, gray_image, mask):
        """
        Detects keypoints by finding top-K brightest pixels within the mask
        and applying Non-Maximum Suppression (NMS).

        Args:
            gray_image: (B, 1, H, W) - grayscale image (0-1 range).
            mask: (B, 1, H, W) - binary mask (0 or 1) indicating valid regions.

        Returns:
            keypoints: (B, N, 2) - Padded (x, y) coordinates, N <= max_keypoints.
            scores: (B, N) - Padded intensity scores corresponding to keypoints.
        """
        B, _, H, W = gray_image.shape
        device = gray_image.device

        keypoints_list = []  # List to store keypoints for each batch item
        scores_list = []  # List to store scores for each batch item

        for b in range(B):
            img = gray_image[b, 0]  # Get single image (H, W)
            m = mask[b, 0]  # Get corresponding mask (H, W)

            # Apply mask to ignore regions outside the valid area
            img_masked = img * m

            # Find top-K brightest points (potential keypoints)
            img_flat = img_masked.view(-1)  # Flatten the image
            # Find more candidates than needed initially to allow NMS filtering
            num_candidates = min(self.max_keypoints * 4, img_flat.numel())
            if num_candidates <= 0:  # Handle edge case of empty image
                keypoints_list.append(torch.zeros(1, 2, device=device))
                scores_list.append(torch.zeros(1, device=device))
                continue
            values, indices = torch.topk(img_flat, num_candidates)

            # Convert 1D indices back to 2D coordinates (y, x)
            y_coords = indices // W
            x_coords = indices % W

            # Filter out points with low intensity (likely background noise)
            ...
            if len(x_coords) == 0:
                # If no valid keypoints found after filtering, add placeholder tensors
                keypoints_list.append(torch.zeros(1, 2, device=device))
                scores_list.append(torch.zeros(1, device=device))
                continue

            # Non-maximum suppression (NMS) to ensure keypoints are spatially separated
            kp = torch.stack([x_coords, y_coords], dim=1).float()  # Combine coordinates (N, 2)
            kp_nms, scores_nms = self.non_maximum_suppression(kp, values)

            # Limit the number of keypoints after NMS
        ...
        # Pad results within the batch to have the same number of keypoints (N)
        max_len = max(len(kp) for kp in keypoints_list) if keypoints_list else 0
        if max_len == 0: max_len = 1  # Handle case where all images in batch are empty

        keypoints_padded = []
        scores_padded = []

        for kp, sc in zip(keypoints_list, scores_list):
            if len(kp) == 0:  # Handle empty tensor case explicitly before padding
                kp = torch.zeros(1, 2, device=device)
                sc = torch.zeros(1, device=device)

            pad_len = max_len - len(kp)
            if pad_len > 0:
                # Pad with zeros if fewer keypoints than max_len
                kp_pad = torch.cat([kp, torch.zeros(pad_len, 2, device=device)], dim=0)
                sc_pad = torch.cat([sc, torch.zeros(pad_len, device=device)], dim=0)
            else:
                # No padding needed if already at max_len or more (already truncated)
                kp_pad, sc_pad = kp[:max_len], sc[:max_len]  # Ensure correct length if truncated
            keypoints_padded.append(kp_pad)
            scores_padded.append(sc_pad)

        # Stack padded lists into batch tensors
    ...
    def non_maximum_suppression(self, keypoints, scores):
        """
        Performs simple Non-Maximum Suppression (NMS) on keypoints.
        Keeps the highest scoring keypoint within a specified radius.
        Operation performed on CPU for stability with varying point counts.

        Args:
            keypoints: (N, 2) - Tensor of keypoint coordinates (x, y).
            scores: (N,) - Tensor of scores corresponding to keypoints.

        Returns:
            final_kp: (M, 2) - Keypoints after NMS (M <= N).
            final_scores: (M,) - Scores after NMS.
        """
        if len(keypoints) == 0:
            return keypoints, scores
        ...
        # Move tensors to CPU for NMS processing
        ...
        # Initialize mask to keep all points initially
        keep_mask = torch.ones(len(kp_sorted), dtype=torch.bool)

        for i in range(len(kp_sorted)):
            # If the current point is already marked for removal, skip it
            if not keep_mask[i]:
                continue

            # Compute Euclidean distance to all subsequent points
            # Compare kp_sorted[i] with kp_sorted[i+1:]
            dist = torch.norm(kp_sorted[i:i + 1] - kp_sorted[i + 1:], dim=1)

            # Find points within the NMS radius
            suppress_mask = dist < self.nms_radius

            # Get indices of points to suppress (relative to the i+1 slice)
            indices_to_check = torch.arange(i + 1, len(kp_sorted))
            indices_to_suppress = indices_to_check[suppress_mask]

            # Mark nearby points with lower scores for removal
            keep_mask[indices_to_suppress] = False

        # Apply the keep_mask to get the final keypoints and scores
...
class DINOv3FeatureExtractor(nn.Module):
    ...
    def get_feature_map(self, image):
        ...
        if patch_tokens.shape[1] != expected_num_patches:
            # print(f"[WARNING] Number of patch tokens ({patch_tokens.shape[1]}) does not match expected ({expected_num_patches}). Check image padding or DINO model behavior.")
            if patch_tokens.shape[1] > expected_num_patches:
                # print(f"  Truncating tokens.")
                patch_tokens = patch_tokens[:, :expected_num_patches, :]
            # Add handling for fewer tokens if necessary, e.g., padding (more complex)
    ...
    def forward(self, image, keypoints):
        ...
        # Use ... for broadcasting across batch and keypoint dimensions
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, positions, image_size):
        ...
        if W <= 1 or H <= 1:
            ...
            return torch.zeros(*positions.shape[:-1], self.dim, device=positions.device)  # Match shape B, N, dim
...
class CrossAttentionLayer(nn.Module):
    ...
    def forward(self, feat_query, feat_kv):
        ...
        try:
            attn_out, attn_weights = self.attn(q, kv, kv, need_weights=False)  # Don't need weights usually
        except Exception as e:
            print(f"[ERROR] Cross-Attention failed: {e}. Q shape: {q.shape}, KV shape: {kv.shape}")
            # Return query features as fallback
            return feat_query, None  # Return None for weights if failed
        ...
        return features, None  # Return None for weights
...
class SparseMatchingNetwork(nn.Module):
    ...
    def __init__(self, cfg: Config):
        ...
        print(f"Initializing SparseMatchingNetwork with {num_layers} attention layers.")  # Debug print

        # Positional encoding module
        self.pos_enc = PositionalEncoding(dim)

        # Separate self-attention stacks for left and right features
        ...
        # Final cross-attention layer for matching
    ...
    def forward(self, desc_left, desc_right, kp_left, kp_right, image_size):
        ...
        # 2. Apply self-attention layers to refine features within each image
        feat_left = desc_left
        feat_right = desc_right
        for i, (self_l, self_r) in enumerate(zip(self.self_attn_left, self.self_attn_right)):
            # print(f" Self Attn Layer {i+1}/{len(self.self_attn_left)}") # Debug
            feat_left = self_l(feat_left, pos_left)
            feat_right = self_r(feat_right, pos_right)
        # feat_left, feat_right now contain context-aware features

        # 3. Apply cross-attention: left features query right features
        # feat_left_enhanced incorporates information from potential matches in feat_right
        feat_left_enhanced, _ = self.cross_attn(feat_left, feat_right)

        # 4. Compute raw matching scores (cosine similarity)
        ...
        # 5. Enforce Epipolar Constraints
        ...
        # Use torch.finfo().min for the lowest possible value in the current dtype
        match_scores_constrained = match_scores.masked_fill(~constraint_mask, torch.finfo(match_scores.dtype).min)

        # 6. Predict Disparity using Soft Argmax (Weighted Average)
        # Apply temperature scaling and softmax to get probabilities
        # High temperature -> sharper probabilities (more confident matches)
        ...
        # (all probabilities in its row were zero due to masking)
...
class FeatureMetricLoss(nn.Module):
    ...
    def forward(self, desc_left, desc_right, match_scores, keypoints_left, disparity, scores_left, constraint_mask):
        ...
        # 1. Must be a valid keypoint detected (score > 0.1)
...
class RectifiedWaveStereoDataset(Dataset):
    """ Loads, rectifies, crops, resizes, and augments stereo image pairs using manually selected ROIs. """
    ...
    def __init__(self, cfg: Config, is_validation=False):
        ...
        try:
            ...
            # --- v2.10 MODIFICATION: Use Unified Hardcoded ROIs ---
            self.roi_left = (723, 505, 1788, 904)  # Unified ROI Left
            self.roi_right = (51, 505, 1681, 904)  # Unified ROI Right (aligned vertically with left)
            print(f"使用统一的左相机 ROI: {self.roi_left}")
            print(f"使用统一的右相机 ROI: {self.roi_right}")
            # --- END MODIFICATION ---
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # 2. ROI Cropping: Extract the valid region after rectification using manual ROIs
            lx, ly, lw, lh = self.roi_left  # Use the hardcoded ROI
            rx, ry, rw, rh = self.roi_right
            # Add boundary checks for ROI
            h_rect_l, w_rect_l = left_rect.shape[:2]
            h_rect_r, w_rect_r = right_rect.shape[:2]

            # Check if ROI is within the bounds of the rectified image
            if ly < 0 or lx < 0 or ly + lh > h_rect_l or lx + lw > w_rect_l or \
                    ry < 0 or rx < 0 or ry + rh > h_rect_r or rx + rw > w_rect_r:
                print(f"警告: ROI 坐标无效或超出校正后图像边界 (idx {idx}, file index {img_idx})。跳过。")
                print(f"  Left ROI: {self.roi_left}, Left Rect Shape: {(h_rect_l, w_rect_l)}")
                print(f"  Right ROI: {self.roi_right}, Right Rect Shape: {(h_rect_r, w_rect_r)}")
                return None
            ...
            # 5. Mask Generation: Create a binary mask based on brightness threshold
            # Used by SparseKeypointDetector to focus on bright points.
...
class SparseMatchingStereoModel(nn.Module):
    ...
    def forward(self, left_gray, right_gray, left_rgb, right_rgb, mask):
        ...
        # 3. Match features using the attention-based matcher network
...
class EvaluationMetrics:
    ...
    def compute_sparse_metrics(disparity, keypoints_left, scores_left):
        ...
        valid_mask = scores_left > 0.1  # Shape (B, N)
...
class Trainer:
    ...
    def __init__(self, cfg: Config):
        ...
        # Use num_workers=0 if debugging dataloader issues, otherwise try > 0 for performance
        num_workers = 0 if sys.platform == 'win32' else 4  # Basic check, adjust as needed
    ...
    def train(self):
        ...
        print(
            f"Config: Resolution={self.cfg.IMAGE_WIDTH}x{self.cfg.IMAGE_HEIGHT}, MaxKpts={self.cfg.MAX_KEYPOINTS}, Layers={self.cfg.NUM_ATTENTION_LAYERS}, LR={self.cfg.LEARNING_RATE:.1e}, SmoothW={self.cfg.SMOOTHNESS_WEIGHT:.1e}")
    ...
    def visualize(self, data, outputs, step, phase):
        ...
        try:
            ...
            if len(kp_valid) > 0:
                ...
                axes[0].scatter(kp_valid[:, 0], kp_valid[:, 1], c='red', s=10, alpha=0.6)  # Smaller dots
...
if __name__ == "__main__":
    ...
    cfg = Config()
    # --- Disable Auto-tune ---
    # Attempt to auto-tune configuration based on GPU memory
    # try:
    #     auto_tune_config(cfg)
    # except Exception as e:
    #     print(f"[警告] 自动调整配置失败: {e}。 使用默认配置。")
    print(f"使用手动配置的分辨率: {cfg.IMAGE_WIDTH}x{cfg.IMAGE_HEIGHT}")
    # --- End Disable Auto-tune ---

    # Sanity check for batch size after auto-tuning (or manual setting)
    if cfg.BATCH_SIZE <= 0:
