# sparse_matching_stereo.py (V2 - Photometric Loss)
#
# --- [ 核心诊断与 V2 修复 ] ---
#
# 诊断: 原始的 `FeatureMetricLoss` (v9) 存在一个根本性缺陷。
# 1. 模糊性: 所有的泡沫标记物在 DINOv3 特征空间中都非常相似。
# 2. 逻辑陷阱: `FeatureMetricLoss` 是一种自监督损失，它鼓励模型将*特征最相似*
#    的点匹配在一起。这导致模型从一开始就学会了“自信地”匹配*几何上错误*但
#    *特征上相似*的泡沫。
# 3. 结果: 训练出的模型是一个“自信的”坏模型，导致推理时
#    (`inference_and_fit_1024.py`) 产生大量噪声点 (局外点)，拟合失败。
#
# V2 解决方案:
# 1. [替换] 移除 `FeatureMetricLoss`。
# 2. [引入] 引入 `PhotometricLoss` (光度重建损失)。
# 3. [新逻辑] 这个新的损失函数不再比较 DINO 特征的相似性。相反，它
#    利用模型预测的视差(disparity)来“扭曲”(warp)右图，并将其与
#    左图的*原始像素*进行比较 (使用 L1 损失)。
# 4. [新目标] 这迫使模型 (DINOv3 + Transformer) 学习生成*几何上正确*的
#    视差，因为只有正确的视差才能最小化像素重建误差。
# 5. [保留] 我们保留了 `SparseKeypointDetector` (斑点检测器) 和
#    `SparseMatchingStereoModel` (DINOv3+Transformer) 的完整架构。
#
# --- [ 调试修改 v9 (保留) ] ---
...
class Config:
    ...
    RUNS_BASE_DIR: str = os.path.join(PROJECT_ROOT, "training_runs_sparse_photometric")  # [V2] 新的日志目录
    ...
    # Training settings
    BATCH_SIZE: int = 1
    LEARNING_RATE: float = 1e-5  # [V4 修复] 再次降低学习率 (原为 3e-5)。
    NUM_EPOCHS: int = 100
    ...
    PHOTOMETRIC_WEIGHT: float = 1.0  # [V2] 现在这是光度损失的权重
    SMOOTHNESS_WEIGHT: float = 0.05  # [V4 修复] 显著提高平滑权重 (原为 0.005)，强力稳定训练。
...
class SparseMatchingStereoModel(nn.Module):
    ...
    def __init__(self, cfg: Config):
        ...
        print(f"[V2] Using NEW Photometric Loss (L1 on patches)")  # [V2] 更新日志
...
class Trainer:
    ...
    def train(self):
        print("\n--- [V2 训练] Starting Sparse Matching Training (BlobDetector + Photometric Loss) ---")  # [V2]
        print(
