...
def generate_clean_params():
    ...
    #   -> 对于夹角较大的相机（你的是16度），必须用 alpha=1 才能保留视角重叠区！
    #   -> 否则(alpha=0)会导致过度放大，彻底失去公共视野。
    ...
    if f_new > 5000:
        print("[ERROR] focal length too large, alpha may not be effective.")
    elif f_new < 2000:
        print("[WARN] focal length too small, image may be over-shrunk.")
    else:
        print("[OK] focal length in reasonable range (2000-4000).")
