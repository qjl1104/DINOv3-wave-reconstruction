...
def preprocess_stereo_pair(l_path, r_path, calib, mask_thresh, device, patch_size=16):
    """
    ...
    # Pad to multiple of model's patch_size (will be set after model load)
    h, w = l_rect.shape
...
def analyze_physics_properties(df, fps=50):
    ...
    plt.axvline(x=40, color='green', linestyle='-', linewidth=2, label='Target: 40')
...
def main():
    ...
    load_model_checkpoint(model, args.model_path, device, strict=False)
    model.eval()
    ...
    for idx in tqdm(sample_indices):
        ...
        r_path = infer_right_path(l_path, args.right_img_dir)
