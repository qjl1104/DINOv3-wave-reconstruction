...
class MyDataset(Dataset):
    def __init__(self, path, transform, sigma, ex):
        self.transform = transform
        self.sigma = sigma

        for _, _, files in os.walk(path):
            self.imgs = [path + file for file in files if Image.open(path + file)] * ex
...
def get_data(batch_size, train_path, val_path, transform, sigma, ex, num_workers=0):
