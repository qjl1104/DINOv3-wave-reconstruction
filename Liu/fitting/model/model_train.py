...
for epoch in range(1000):
    ...
    nn = nn // 4
    num = epoch % nn
    fx = [2570.5, 480.9, 586.3, 372.3, 649.3, 1128.8, 1704.9, 911]
    factx = (fx - minx) / (maxx - minx)
    factx = factx[:, np.newaxis]
    factx = np.array(factx)
    factx = factx[2 * num:2 * num + 3]
    factx = torch.tensor(factx, dtype=torch.float)

    fy = [42.4, 9.5, 1.1, 20.8, -3, -40, -20.6, -25.6]
    facty = (fy - miny) / (maxy - miny)
    facty = facty[:, np.newaxis]
    facty = np.array(facty)
    facty = facty[2 * num:2 * num + 3]
    facty = torch.tensor(facty, dtype=torch.float)

    loss = None
    for batch_x, batch_y in dataloader:
        y_predict = net(batch_x)
        y_fact = net(factx)
        loss = Loss(y_predict, batch_y) + Loss(y_fact, facty)
...
predict1 = net(torch.tensor(ds_x, dtype=torch.float))
predict2 = net(torch.tensor(factx, dtype=torch.float))
p1 = predict1.detach().numpy()
p2 = predict2.detach().numpy()
p_p1 = p1 * (maxy - miny) + miny
p_p2 = p2 * (maxy - miny) + miny
ds_y = ds_y * (maxy - miny) + miny
ds_x = ds_x * (maxx - minx) + minx
np.save("y_tra1.npy", p_p1)
np.save("y_tra2.npy", p_p2)
np.save("x_ini.npy", ds_x)
np.save("y_ini.npy", ds_y)
np.save("x_true.npy", fx)
np.save("y_true.npy", fy)

import matplotlib.pyplot as plt

plt.plot(ds_x, ds_y, label="fact")
plt.plot(ds_x, (predict.detach().numpy()) * (maxy - miny) + miny, label="predict")
plt.scatter(fx, fy, edgecolors='r')
plt.title("function")
plt.xlabel("x")
plt.ylabel("sin(x)")
plt.legend()
