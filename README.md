...
原始图像 (left/right, 1000帧@50Hz)
  ...
  → 相干性验证（互谱相位法：c=1900 mm/s ✓）
  → PINN（互谱测向旋转传播坐标系 + 固定 c=1976，留出集 R²=0.65–0.71）
  → 波成分场（0.6–1.0Hz 带通：f=0.797Hz, λ=2443mm, 波成分 rms 20.0 vs 21.2mm）
```
...
（互谱相位 c=1900 mm/s、CI [1812,1986]、RMS 128ms，与 v2 同等相干）。
...
- 控制方程：线性浅水波方程 η_tt = c²∇²η（c = √(gh)），
  可选 Boussinesq 色散修正 +(h²/3)∇²η_tt（逼近完整 Airy 色散
  ω² = gk·tanh(kh) 到 O((kh)²) 阶）
...
（ξ 沿传播，各向异性 Fourier sigma=(1.0,0.3,1.0)）→ 20% 留出评估。

- 留出集 **R² = 0.65–0.71**（c 固定为理论值 1976 mm/s）
...
DINO 描述子（缓存则跳过，`--force-desc` 重算）→ `rematch_dino_v2.py` →
`eval_tracks.py` 相干性 → `run_real_pinn.py` → `final_visualize.py`。

最终重建（`wave_modeling/real_run/final_result.png` + `final_field.npz`）：

- 波成分（0.6–1.0Hz 带通）：主频 **0.797 Hz**（理论 0.79）、
  波长 **2443 mm**（理论 2502）、数据点处波成分 rms **20.0mm vs 数据
  21.2mm**（理论 A=40mm→rms≈28mm，链路捕捉 ~70%，其余为噪声/低频晃动）
...
- 快速核验：`verify_results.py` → `verify_results.png`
  ...
  带通后主峰回 ~0.8Hz、PINN 波成分逐段 R² 0.5–0.86）
...
**c = 1975 mm/s（95% CI [1867, 2107]）**，与深水理论值 1976 几乎重合，
...
**已完成的真值验证**（无需新实验的替代手段）：
1. 长轨迹 η(t) FFT 主峰 0.75–0.81 Hz（带通后中位 0.792 Hz，与 0.79 Hz 一致）、
   振幅估计 21–43mm（与 A=40mm 一致）→ 粒子链路确实测到了论文规则波。
2. 互谱相位法测速 c = 1975 mm/s（95% CI [1867, 2107]）= 深水理论值
   （见"验证体系"节）。
3. 重建场波成分：0.797 Hz / 2443mm / 波成分 rms 20.0 vs 21.2mm
   （见"全链路复现"节）。
