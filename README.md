...
原始图像 (left/right, 1000帧@50Hz)
  → 预处理 + blob 检测（亚像素质心）
  → 单相机 EKF 时序跟踪（2D 轨迹）→ **跳切清洗**（治跨泡沫跳号）
  → 跨相机匹配（几何判据 + 视差平面先验 + DINOv3 描述子验证/救回）
  → 三角测量（3D 轨迹片段 237 条 / 11966 点）
  → 相干性验证（互谱相位法：c = 2033 mm/s，95% CI [2000,2070]，理论 1976）
  → PINN（互谱测向旋转传播坐标系 + c 固定为互谱实测 1956，片段级留出 R²=0.456）
  → 波成分场（0.6–1.0Hz 带通：f=0.759Hz, λ=c/f=2579mm, 振幅 23.4mm）
```

> **⚠ 2026-07-23 最终修复结论（四层 bug 连环）**：本轮 review 连环挖出
> 四层 bug，均已修复并全链路验证（真值锚点全部归位）：
...
> ④ **2D 跟踪器本身跨泡沫跳号**（纯距离关联，dist_thresh=80px > 泡沫间距
> ~50px：实测左 97%、右 88% 轨迹含 >12px 非物理跳变）——
> `clean_tracks_jumpcut.py` 在 >12px 跳变处切成身份干净段，
> 生产输入切换为 `trajectories_2d_*_jumpcut.pkl`。
> **修复后真值锚点全部归位**：c = 2033 mm/s（理论 1976，方向先验独立测
> 1956，夹住理论）、f = 0.759–0.797 Hz（0.79）、λ = c/f = 2579mm（2502）、
> 去偏 η std 27.9mm（正弦理论 28.3）、波成分 rms 22.3mm、
> 相位残差 RMS 162→24ms、轨迹间偏移 560→19mm、匹配 79→237 对、
> PINN 片段级留出 R² = **0.456**（修复前为负）。
...
### 3. 单相机 2D 时序跟踪 `particle_processing/03_*` + 跳切清洗
...
⚠ **跳号污染与跳切清洗（2026-07-23）**：03 关联只用距离
（dist_thresh=80px > 泡沫间距 ~50px），断档重捕常抓错泡沫——实测
左 97%、右 88% 的轨迹含 >12px 非物理跳变（泡沫轨道速度上限 ~2px/帧，
个例跳 304px）。`clean_tracks_jumpcut.py` 在 >12px（跨断档按 4px/帧放宽）
跳变处切开、丢弃 <20 帧碎段，产出身份干净的
`trajectories_2d_*_jumpcut.pkl`（左 144→767 段、右 285→1201 段，
中位 34–37 帧）——**生产链默认输入已切换为它**；原始 `*_optimized.pkl`
保留存档。效果：匹配片段 79→237、相位残差 RMS 162→24ms、
η std 47.5→27.9mm、PINN 留出 R² 0.196→0.456。
覆盖率说明：每帧 ~340 泡沫中仅 ~12% 被确认轨迹（保守确认机制 +
纯距离关联的固有上限）；提高覆盖需重跑检测+跟踪（待 08-07 代预处理
复现），届时关联应加入外观特征（patch NCC/KLT）而非纯距离。
...
跳切轨迹 237 个几何候选全部通过（med 0.733 ≥ 0.55）
→ **237 条片段 / 11966 点** → `data/trajectories/trajectories_3d_v2_dino.pkl`
（互谱相干性验证见"验证体系"节：c = 2033 mm/s，CI [2000,2070]）。
...
- **c 固定为互谱实测值（当前 1956 mm/s，`--c` 可覆盖）**：可学习时反演
  不可辨识（曾被噪声宽带模态拖走），故 c 由互谱相位法独立测量、
  物理损失只承担时空相干性约束。跳切轨迹后两个独立测量（方向先验 1956 /
  相干性评估 2033）把理论值 1976 夹在中间。
- **片段级留出评估（2026-07 起）**（190 训练 / 47 测试段，seed 固定；
  PCA 平面、MAD 阈值、归一化 bounds 仅拟合训练侧——旧版随机点切分只测
  插值能力、且上述统计在全池数据上拟合存在泄漏）。
  **当前结果：留出片段 R² = 0.456**（train η std 27.2 / test 26.6mm，
  data loss 0.151）——跳切修复后由 0.196 再翻一倍多，逼近 0.5 良好线；
  之前 R²<0 的根因是跨相机错配 + 2D 跳号污染空间位置，不是模型问题。
  进一步改进方向：提高匹配亚像素精度、只用长片段训练、
  调整 PINN 容量/物理权重。
- 已知系统误差：轨迹间存在 ~19mm（std）常值深度偏移（曾 560→128→37mm，
  跳切后再降到 19mm——"碗形"的大头都是错配扰乱），用逐片段
...
- 波成分（0.6–1.0Hz 带通）：主频 **0.759 Hz**（理论 0.79，略低）；
  **λ = c/f = 2579mm**（理论 2502）✓；中心点振幅 **23.4mm**（理论 40，
  链路有衰减）；波成分 rms **22.3mm**；原始场 rms 26.2mm（慢变结构
  已很小）；数据点去偏 η std 27.9mm（正弦理论 28.3）✓。
  （λ 用 c/f 换算：空间 FFT 只作定性参考；振幅类数字经 Hann 窗增益修正。）
...
**基线判决（2026-07-23 跳切修复后重测）**：粒子链有效轨迹对的相位差对
沿传播方向距离呈干净线性，**c = 2033 mm/s（95% CI [2000, 2070]，残差 RMS
24ms；方向先验独立测得 1956——两个独立估计把理论值 1976 夹在中间）**，
传播方向在 PCA 面内 (0.682, −0.731) 轴。互谱方法学的测量史值得铭记：
① 旧三角化 bug 几何下 c≈1900–1975（"吻合"是巧合）；② 三角化修好但
匹配仍错配时 c≈720（幻影，曾据此误判"深水被排除、浅水 h≈57mm"）；
③ 视差平面先验消歧后 c=2048（RMS 162ms）；④ 再经 2D 跳切清洗后
c=2033，残差降到 24ms——与理论/浪高仪一致。教训：**相位法只测量
"相位差 ÷ 空间距离"，空间距离错则一切皆错；它判得了方法，判不了匹配**。
全链路几何正确性的底层证据：重投影误差 ~1.3px + 标记物尺寸回归同构
（见第 4 节）+ 去偏 η std 27.9mm 落在正弦理论 28.3mm 上。
...
原始输出，**存档**——实测含跨泡沫跳号污染）；
`trajectories_2d_*_jumpcut.pkl`（跳切清洗版，**生产默认输入**，可由
`clean_tracks_jumpcut.py` 重新生成）；
`trajectories_3d_v2.pkl`、`trajectories_3d_v2_dino.pkl`（下游产物，
重跑生产链可得）。
