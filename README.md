...
原始图像 (left/right, 1000帧@50Hz)
  ...
  → 三角测量（3D 轨迹片段 83 条 / 16897 点）
  → 相干性验证（互谱相位法：c = 720 mm/s，95% CI [694,749]，方向 −v）
  → PINN（互谱测向旋转传播坐标系 + c 固定为互谱实测 746，片段级留出评估）
  → 波成分场（0.6–1.0Hz 带通：f≈0.78Hz, λ=c/f≈935mm）
```

> **⚠ 2026-07-22 代码修复与重跑结论**：本轮 review 修复了一批影响数值正确性的
> 问题并已全链路重跑。**两项最重要的结果**：
> ① 三角化坐标系混用已修（`rematch_rectified.triangulate_pairs`）：真实匹配点对
> 重投影误差 **1.46px vs 旧方法 498px**，新几何确凿正确；轨迹间常值偏移
> （"碗形"）从 ±300~560mm 降到 128mm，去偏后 η std 16.4mm。
> ② 正确几何下互谱相位法测得 **c ≈ 720–746 mm/s**——深水理论 1976 被数据
> 明确排除（沿传播向相位梯度为其 2.66 倍，平面波表观速度不可能低于真实 c）；
> 旧几何下"c=1975≈理论"是倾斜方向被拉伸 2.6 倍的**巧合**（1876/720≈2.6）。
> 按 Airy 色散反推对应**浅水 h≈55–60mm**，说明这份数据很可能不来自论文那组
> 深水大水池实验（详见"真值锚点"节），数据来源待与实验方确认。
> 其余修复：反向视差 clamp 方向化、监督损失用未 clamp 视差、几何指纹排除自身、
> checkpoint 键核对告警、描述子半 patch 对齐、片段级留出防泄漏、断帧门控重
> 网格化、Hann 窗振幅 ×2 修正、λ 改报 c/f。PINN 片段级留出 R² 目前为负——
> 见第 5 节的诚实评估与原因分析。
...
**精度链条的物理约束**：正确几何下本装置 Z≈3.2–5.3m，**每 ±1px 的视差/定位
误差 ≈ ±5mm 深度噪声**（δZ = Z²/(bf)·δd，Z=4.3m、bf=3.72e6），波幅 ~20–40mm
——仍是**亚像素精度的游戏**（旧错误几何 Z≈6–19m 时曾高估为 ±34mm/px）。
这一条物理约束判决了所有技术路线的可行性（见"路线判决记录"）。
...
- 标定重投影误差决定整个几何链的精度上限
- 基线 1413mm，矫正后视差约 350–1170px（主峰 800–1000，对应 Z≈3.7–4.5m）
...
**修复正确性的决定性证据**：真实 83 对匹配 / 16897 点对的重投影误差——
新方法 **1.46px（p90 14.4）**，旧方法 **498px（p90 874）**，差 2.5 个数量级。
重跑后片段保留率 83/83（旧 bug 把大量点的 Z 顶出滤波区间），
轨迹间常值偏移（"碗形"）从 ±300~560mm 降到 **128mm**。
...
候选中救回 4 对 → **83 条片段 / 16897 点**
→ `data/trajectories/trajectories_3d_v2_dino.pkl`
（互谱相干性验证见"验证体系"节：c = 720 mm/s，CI [694,749]）。
...
- 控制方程：线性波动方程 η_tt = c²∇²η，c 由互谱相位法实测给定
  （本数据 c≈730 mm/s，对应浅水色散；深水 c=g/ω=1976 已被数据排除，
  见"验证体系"节）。可选 Boussinesq 色散修正 +(h²/3)∇²η_tt
...
- **c 固定为互谱实测值（746 mm/s，`--c` 可覆盖）**：可学习时反演不可辨识
  （曾被噪声宽带模态拖到 772 mm/s 且持续下滑），故 c 由互谱相位法独立测量、
  物理损失只承担时空相干性约束。c=746 下 PDE 残差比 c=1976 小 ~40 倍——
  波动方程本身也证实 730 量级是正确物理。
- **片段级留出评估（2026-07 起）**（66 训练 / 17 测试段，seed 固定；
  PCA 平面、MAD 阈值、归一化 bounds 仅拟合训练侧——旧版随机点切分只测
  插值能力、且上述统计在全池数据上拟合存在泄漏）。
  **当前诚实结果：留出片段 R² < 0**。原因已定位（平面波基线实验，
  2 参数全局平面波在同一切分上 test R²≈+0.2 > PINN 的 −0.45）：
  每条片段自身在 0.78Hz 都有 5–16mm 的真实振荡，且相位与全局平面波
  基本一致（散布 ±30°），但片段间剩余失配 + 片段内的视差噪声结构
  （~30px ↔ ~150mm，数倍于波幅）让 PINN 的高容量场**过拟合片段噪声**。
  即：相位速度测量是稳健的，而**场级重建目前受限于匹配精度，不可信**；
  旧随机切分的 "R² 0.65–0.71" 只是同片段插值能力。
  改进方向：更严的片段内视差门（如 MAX_DISP_STD 30→10）、只用长片段训练、
  提高匹配亚像素精度、或降低 PINN 容量/加强物理权重。
- 已知系统误差：轨迹间存在 ~128mm（std）常值深度偏移（三角化修复前为
  ±300~560mm——那部分主要是 bug；剩余 128mm 为标定/匹配残差），用逐片段
...
- 波成分（0.6–1.0Hz 带通）：长片段（≥300帧）主频中位 **0.781 Hz**；
  **λ = c/f ≈ 935mm**（c≈730 互谱实测 × 实测主频；ξ 孔径 ~921mm ≈ 1λ，
  空间 FFT 仍只作定性参考）；数据点处去偏 η std 16.4mm。
  振幅类数字已经 Hann 窗增益修正（旧版 2|X|/N 系统性偏小 2 倍）。
  **注意：PINN 波成分场目前不可信**（片段级留出 R²<0，见第 5 节）——
  `final_result.png` 的 PINN 场仅供参考，可信的是数据侧统计与 c 测量。
...
- 快速核验：`verify_results.py` → `verify_results.png`
  ...
  带通后主峰回 ~0.78Hz；PINN 逐段 R² 目前为负，见第 5 节分析）
...
**断帧处理（2026-07 修）**：82/83 条片段存在帧空洞（实测 1264 个、最长
24 帧≈0.37 个波周期）。FFT/带通前先按真实帧号重网格化：≤4 帧空洞线性插值，>4 帧空洞
...
**基线判决（2026-07 三角化修复后重测）**：粒子链 967 个轨迹对的相位差对
沿传播方向距离呈干净线性，**c = 720 mm/s（95% CI [694, 749]，残差 RMS
176ms；仅 ≥300 帧长片段：c = 731，RMS 74ms）**，传播方向沿 PCA 面内 −v 轴
（沿 u 向相位梯度 ≈0，符合平面波）。

**深水理论 1976 mm/s 已被数据排除**：实测相位梯度 1.346 s/m 是 1976 允许的
0.506 的 2.66 倍，而平面波沿任何方向的表观速度都不可能低于真实 c——
与方向估计无关。旧错误几何下曾测得 c≈1876–1975 并以为"与理论吻合"，
实为旧三角化把倾斜方向拉伸 2.6 倍的**巧合**（1876/720≈2.6）。
按 Airy 色散 ω²=gk·tanh(kh) 反推，f=0.781Hz + λ≈935mm 对应
**水深 h≈55–60mm**（c≈√(gh)≈748，近非色散）——这强烈提示本数据并非
论文那组深水大水池实验（见"真值锚点"节）。已知残留张力：仅近距对
（300–400mm，无需解缠绕）估计 1005–1294，高于全对 720——杠杆短噪声大，
但也可能暗示非理想平面波（水槽反射/驻波成分），驻波检验列为 TODO。
全链路几何正确性的证据改为**重投影误差 1.46px**（见第 4 节），
这比"c 凑巧吻合理论"硬得多。
...
本项目数据曾认为与下列参数同出一源（原始图像 2560×1600 = Phantom VEO-E340L
原生分辨率，仅确证相机型号）。**2026-07 三角化修复后，该锚定被数据动摇，
见本节末尾的存疑分析**：
...
**真值核验（2026-07 重测）与数据来源存疑**：
1. 频率一致：长轨迹 η(t) 主峰中位 **0.781 Hz**（论文 0.79）✓。
2. 振幅量级一致：去偏 η std 16.4mm、单片段 0.78Hz 振幅 5–16mm
   （论文 A=40mm，链路对振幅有衰减）~✓。
3. **波长/波速不一致 ✗**：实测 c≈730 mm/s、λ≈935mm，
   论文深水波 λ=2.5m / c=1976。平面波表观速度不可能低于真实 c，
   无法用方向误差解释；按 Airy 色散反推本数据对应 **h≈55–60mm 浅水**。
   大水池（池深 7.5m）不可能有此色散。**结论：这份数据很可能不来自
   论文那组深水实验**——频率 0.79 的巧合曾误导锚定。可能为同一对
  Phantom 相机拍摄的小型水槽/预实验（h≈5–6cm 与 A≈2–4cm 自洽：
   H/h≈0.8 接近破碎上限的强非线性波，这也可能是片段级 PINN 拟合
   困难的另一原因）。**TODO：与实验方确认本数据的实际装置与水深**；
   若确认为浅水，则 PINN 应换用浅水/非线性色散模型（Boussinesq 已备）。
...
| 三角化 | 位置相关深度偏差（修复后 ~128mm std） | 逐片段 debias；根治需重标定/静水参考帧 |
| PINN | 线性波动方程 + 固定 c（实测） | 浅水/非线性需换 Boussinesq 或更高阶模型 |
...
确认无误后可删）。注意这些 pkl **未被 git 追踪**，再次重跑前务必备份。
...
rematch，仅得 6 条片段 / 741 点（当前 v2 为 79 条 / 16383 点）。
