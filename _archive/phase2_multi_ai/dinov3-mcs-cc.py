...
# (V8 最终修正版：修正代价相关性层的信息丢失问题，旨在最大化模型性能)
...
class DINOv3StereoModel(nn.Module):
    def __init__(self, cfg: Config):
        ...
        self.disp_preds = nn.ModuleList([
            nn.Conv3d(32 * 4, 1, 3, padding=1),
            nn.Conv3d(32 * 2, 1, 3, padding=1),
            nn.Conv3d(32 * 1, 1, 3, padding=1),
        ])

        self.feature_projection = nn.Conv2d(self.feature_dim, 32, 1, bias=False)
        print("模型构建完成 (代价相关性 + 3D沙漏网络架构)。")
    ...
    # [核心修正] 修正代价相关性层，保留完整的特征信息
    def build_cost_volume(self, left_feat, right_feat):
        """Build cost volume by correlating features"""
    ...
    def forward(self, left_image, right_image):
        ...
        left_feat = self.feature_projection(left_feat)
        right_feat = self.feature_projection(right_feat)
