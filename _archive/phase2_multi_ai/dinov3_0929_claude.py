# self_supervised_dinov3_raft_optimized.py
# An end-to-end, self-supervised 3D reconstruction pipeline for dynamic wave surfaces.
# OPTIMIZED ARCHITECTURE: Fixed disparity range and prediction mechanism
# KEY IMPROVEMENTS:
# 1. Changed from 2-channel flow to single-channel disparity prediction
# 2. Used sigmoid activation to ensure positive disparity values
# 3. Increased disparity update scale from 1.0 to 50.0
# 4. Added disparity range constraints (0-100 pixels)
# 5. Improved initialization with reasonable initial disparity
# 6. Fixed MotionEncoder to accept 1-channel disparity input
...
class Config:
    ...
    # KEY FIX: Increased disparity scale and added range constraints
    DISP_UPDATE_SCALE: float = 50.0
    MIN_DISPARITY: float = 0.0
    MAX_DISPARITY: float = 100.0
    INITIAL_DISPARITY: float = 20.0
...
def auto_tune_config(cfg: Config):
    ...
    print(
        f"--- Auto-tuned config applied: ---\n  Resolution: {cfg.IMAGE_WIDTH}x{cfg.IMAGE_HEIGHT}, Batch Size: {cfg.BATCH_SIZE}, Iterations: {cfg.ITERATIONS}, Grad Accum: {cfg.GRADIENT_ACCUMULATION_STEPS}\n---------------------------------")
    return cfg
...
class MotionEncoder(nn.Module):
    def __init__(self):
        ...
        corr_ch = (2 * 4 + 1) ** 2 * 4
        # FIX: Changed to 1-channel disparity input
        self.conv1 = nn.Sequential(nn.Conv2d(1 + corr_ch, 128, 3, 2, 1), nn.GroupNorm(8, 128), nn.ReLU(True))
...
class UpdateBlock(nn.Module):
    def __init__(self, scale=50.0, min_disp=0.0, max_disp=100.0):
    ...
    def forward(self, net, inp, corr, disp):
        ...
        # KEY FIX: Use sigmoid to ensure positive values
        delta_disp = torch.sigmoid(self.disp_head(net)) * self.scale
...
class ImprovedSelfSupervisedLoss(nn.Module):
    ...
    def compute_photometric_loss(self, w, t, m):
        ssim_loss = (self.ssim(w, t) * m).sum() / (m.sum() + 1e-8)
        l1_loss = (torch.abs(w - t) * m).sum() / (m.sum() + 1e-8)
        return self.photometric_weights[0] * ssim_loss + self.photometric_weights[1] * l1_loss
...
class RectifiedWaveStereoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        try:
            ...
            # Save first rectified pair for debugging
            if not self.debug_saved and not self.is_validation:
                debug_dir = os.path.join(self.cfg.RUNS_BASE_DIR, "debug")
                os.makedirs(debug_dir, exist_ok=True)
                cv2.imwrite(os.path.join(debug_dir, "debug_left_rect.png"), left_rect)
                cv2.imwrite(os.path.join(debug_dir, "debug_right_rect.png"), right_rect)
                print(f"\n[DEBUG] Saved rectified images to {debug_dir}")
                print(f"[DEBUG] Left ROI: {self.roi_left}, Right ROI: {self.roi_right}")
                print(f"[DEBUG] Rectified shape: {left_rect.shape}")
...
class DINOv3StereoModel(nn.Module):
    def __init__(self, cfg: Config):
        ...
        self.update_block = UpdateBlock(scale=self.cfg.DISP_UPDATE_SCALE,
                                        min_disp=self.cfg.MIN_DISPARITY,
                                        max_disp=self.cfg.MAX_DISPARITY)
        print("RAFT-style model with fixed disparity prediction built (DINOv3 FROZEN).")
    ...
    def initialize_disparity(self, image):
        """Initialize with reasonable disparity value"""
        b, _, h, w = image.shape
        h_feat, w_feat = h // self.patch_size, w // self.patch_size
        return torch.ones(b, 1, h_feat, w_feat, device=image.device) * self.cfg.INITIAL_DISPARITY
    ...
    def forward(self, left_image, right_image):
        ...
        # KEY FIX: Single-channel disparity
        disp = self.initialize_disparity(left_image)
        ...
        for _ in range(self.cfg.ITERATIONS):
            disp = disp.detach()
            # KEY FIX: Only x-direction offset
            coords1 = coords0.clone()
            coords1[:, 0, :, :] -= disp.squeeze(1)

            corr = corr_fn(coords1)
            net, delta_disp = self.update_block(net, inp, corr, disp)

            disp = disp + delta_disp
            disp = torch.clamp(disp, self.cfg.MIN_DISPARITY, self.cfg.MAX_DISPARITY)
            preds.append(disp)
...
class Trainer:
    def __init__(self, cfg: Config):
        ...
        self.optimizer = optim.AdamW(filter(lambda p: p.requires_grad, self.model.parameters()), lr=cfg.LEARNING_RATE, weight_decay=1e-4)
    ...
    def train(self):
        ...
        print(f"[CONFIG] Disparity range: [{self.cfg.MIN_DISPARITY}, {self.cfg.MAX_DISPARITY}]")
        print(f"[CONFIG] Update scale: {self.cfg.DISP_UPDATE_SCALE}, Initial disparity: {self.cfg.INITIAL_DISPARITY}")
        best_val_psnr = 0.0
        epochs_no_improve = 0
        ...
        for epoch in range(self.cfg.NUM_EPOCHS):
            ...
            print(f"Epoch {epoch + 1}/{self.cfg.NUM_EPOCHS} -> Train Loss: {train_results.get('total', 0.0):.4f} | Val PSNR: {avg_val_psnr:.2f}")
            ...
            if avg_val_psnr > best_val_psnr:
                ...
                print(f"Val PSNR improved to {best_val_psnr:.2f}. Model saved.")
            else:
                epochs_no_improve += 1
    ...
    def _run_epoch(self, epoch, is_training):
        ...
        for data in pbar:
            ...
            with torch.amp.autocast('cuda', enabled=self.cfg.USE_MIXED_PRECISION):
                disp_preds = self.model(left, right)
            ...
            disp_stats['min'].append(final_disp.min().item())
            disp_stats['max'].append(final_disp.max().item())
            disp_stats['mean'].append(final_disp.mean().item())
            ...
            metrics = self.evaluator.evaluate_reconstruction({"left_image": left}, {"disparity": final_disp}, loss_comps)
            ...
            pbar.set_postfix({
                ...
                'disp_mean': final_disp.mean().item()
            })
            ...
            if is_training:
                ...
                if self.cfg.VISUALIZE_TRAINING and self.step % self.cfg.VISUALIZE_INTERVAL == 0:
                    self.visualize(left, mask, final_disp, loss_comps, self.step, "train")
                self.step += 1

        num_batches = len(loader)

        if disp_stats['mean']:
            avg_disp_min = np.mean(disp_stats['min'])
            avg_disp_max = np.mean(disp_stats['max'])
            avg_disp_mean = np.mean(disp_stats['mean'])
            phase_name = "Train" if is_training else "Val"
            print(f"\n[{phase_name} Disparity Stats] Min: {avg_disp_min:.2f}, Max: {avg_disp_max:.2f}, Mean: {avg_disp_mean:.2f}")

            if self.writer:
                self.writer.add_scalar(f'Disparity/{phase_name}_min', avg_disp_min, epoch)
                self.writer.add_scalar(f'Disparity/{phase_name}_max', avg_disp_max, epoch)
                self.writer.add_scalar(f'Disparity/{phase_name}_mean', avg_disp_mean, epoch)
    ...
    def _log_epoch_results(self, phase, epoch, results):
        for k, v in results.items():
            ...
            if self.writer:
                self.writer.add_scalar(f"{'Loss' if k in self.loss_keys else 'Metrics'}/{phase}_{k}", v, epoch)
        ...
        if self.writer and phase == 'val' and self.history['train']['total']:
            ...
            if 'psnr' in self.history['train'] and self.history['train']['psnr']:
                self.writer.add_scalars('PSNR/epoch_comparison',
                                       {'train': self.history['train']['psnr'][-1], 'val': results.get('psnr', 0)},
                                       epoch)
    ...
    def visualize(self, left_tensor, mask_tensor, final_disp, loss_components, step, phase):
        ...
        fig.suptitle(f'RAFT Model - Step: {step} ({phase}) | Disp Range: [{pred_disp.min():.1f}, {pred_disp.max():.1f}]', fontsize=16)
        ...
        axes[1, 0].set_title(f"Predicted Disparity (Mean: {pred_disp[mask > 0].mean():.1f})")
