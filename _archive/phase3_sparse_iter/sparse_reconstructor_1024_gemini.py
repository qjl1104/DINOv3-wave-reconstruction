...
def auto_tune_config(cfg: Config):
    """ Automatically adjust batch size and resolution based on available GPU memory. """
    ...
    elif free_mem_mb < 12000:  # Medium memory
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, positions, image_size):
        ...
        if W <= 1 or H <= 1:
            ...
            return torch.zeros_like(positions[..., :self.dim])  # Match shape B, N, dim
...
class SparseMatchingStereoModel(nn.Module):
    ...
    def forward(self, left_gray, right_gray, left_rgb, right_rgb, mask):
        ...
        # Use a simple threshold mask for right keypoints as well? Or just ones?
        # Using ones is simpler if right mask isn't critical.
...
if __name__ == "__main__":
    ...
    # Sanity check for batch size after auto-tuning
    if cfg.BATCH_SIZE <= 0:
        print("[警告] 自动调整后的批次大小 <= 0。 重置为 1。")
        cfg.BATCH_SIZE = 1
