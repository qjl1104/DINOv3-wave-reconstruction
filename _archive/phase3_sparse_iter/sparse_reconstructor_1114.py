...
class Trainer:
    ...
    def __init__(self, cfg: Config):
        ...
        # =========================================================================
        # --- [START] MODIFICATION: Val Loader Bug 的最终修复 ---
        # =========================================================================
        # 训练加载器使用 4 个进程以保证速度
        train_num_workers = 4
        # 验证加载器必须使用 0 个进程以避免 Windows 上的迭代器重置 Bug
        val_num_workers = 0
        print(f"[Trainer Config] 强制设置 num_workers (Train={train_num_workers}, Val={val_num_workers})")
        ...
        self.train_loader = DataLoader(train_ds, batch_size=cfg.BATCH_SIZE, shuffle=True,
                                       ...
                                       persistent_workers=True if train_num_workers > 0 else False)  # 训练保持开启
        ...
        #
        # 最终修复: 即使 persistent_workers=False, num_workers > 0
        # 在 Windows 上的 val_loader 仍有问题。
        # 100% 可靠的修复是强制 num_workers = 0。
        self.val_loader = DataLoader(val_ds, batch_size=cfg.BATCH_SIZE, shuffle=False,
                                     collate_fn=collate_fn, num_workers=val_num_workers,  # <--- 必须为 0
                                     pin_memory=True if self.device.type == 'cuda' else False,
