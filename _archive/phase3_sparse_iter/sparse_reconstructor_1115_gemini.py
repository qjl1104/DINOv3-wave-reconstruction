...
#
# --- [ 关键修改 ] ---
# 1. 将 'SparseKeypointDetector' 替换为 1030 脚本中的 'cv2.SimpleBlobDetector'
# 2. 将 'RectifiedWaveStereoDataset' 中的 'resize' 逻辑替换为 'crop' 到最小公共 ROI
# 3. 移除了 'auto_tune_config'
#
# --- [ 调试修改 v9 - 双重修复 ] ---
# 1. (来自 v8) 修复了数据增强 Bug，Detector 现在只使用原始灰度图。
# 2. (来自 v8) 放宽了 BlobDetector 参数 (maxInertia, minConvexity)
#    以同时捕获 "远处的椭圆" 和 "近处的过曝亮团"。
# 3. [新] 修复了 Trainer.visualize 中预览图模糊的问题。
#    通过提高 figsize 和 dpi 来生成高分辨率、清晰的预览图。
#
# 这个脚本现在使用精确的斑点检测器进行训练，以消除噪声。
...
class Config:
    ...
    # --- [ 调试 v8/v9: 斑点检测器参数 ] ---
    # 目标: 捕获 "椭圆" (低惯性) 和 "亮白团" (高惯性)

    BLOB_MIN_THRESHOLD: float = 30.0

    # 1. 面积 (Area)
    BLOB_MIN_AREA: float = 20.0  # (保持 20.0, 捕获远处的点)
    BLOB_MAX_AREA: float = 2000.0  # (放宽上限, 应对近处的亮团)

    # 2. 圆度 (Circularity)
    BLOB_MIN_CIRCULARITY: float = 0.1  # (保持 0.1)

    # 3. 凸性 (Convexity)
    BLOB_MIN_CONVEXITY: float = 0.90  # (保持 0.90)

    # 4. 惯性比 (Inertia Ratio) - 核心过滤器
    BLOB_MIN_INERTIA: float = 0.1  # (保持 0.1)
    # [v8] 大幅放宽惯性上限, 允许 "亮白团" (高惯性) 通过
    BLOB_MAX_INERTIA: float = 0.85  # (原为 0.65)
    # --- [ 修改结束 ] ---
...
# --- 1. [ 关键替换 ] ---
# 使用来自 'sparse_reconstructor_1030_gemini.py' 的 Blob 检测器
...
class FeatureMetricLoss(nn.Module):
    ...
    # [ v7 修复 ]
    # 删除了 v6 错误添加的 'mask' 参数。
    # loss_fn 应该只使用 'scores_left' (斑点大小) 来进行掩码。
...
# --- 5. [ v8 关键修复 ] ---
# 修复了数据增强 (Augmentation) 污染检测器 (Detector) 输入的 Bug.
class RectifiedWaveStereoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        try:
            ...
            return {
                'left_gray': left_gray_tensor,  # (来自原始图, 用于 Detector)
                'right_gray': right_gray_tensor,  # (来自原始图, 用于 Detector)
                'left_rgb': left_rgb_tensor,  # (来自增强图, 用于 DINO)
                'right_rgb': right_rgb_tensor,  # (来自增强图, 用于 DINO)
                'mask': mask_tensor  # (来自原始图, 用于 Loss)
            }
        except Exception as e:
...
class Trainer:
    ...
    def train(self):
        print("\n--- [新训练] Starting Sparse Matching Training (BlobDetector + FeatureMetric Loss) ---")
        print(
    ...
    def visualize(self, data, outputs, step, phase):
        ...
        try:
            ...
            fig.suptitle(f'Sparse Matching (BlobDetector) - Step: {step} ({phase})', fontsize=16)
            # --- [ v9 修复结束 ] ---
