...
def calibrate_stereo_camera(image_dir_left, image_dir_right, chessboard_size, square_size, params_save_path):
    ...
    for i, (fname_left, fname_right) in enumerate(zip(images_left, images_right)):
        ...
        ret_left, corners_left = False, None
        ret_right, corners_right = False, None
        used_method = None

        # 依次尝试不同的检测方法
        for method in detection_methods:
            # 预处理图像
            processed_left = method['preprocess'](gray_left.copy())
            processed_right = method['preprocess'](gray_right.copy())

            # 尝试检测角点
            flags = method['flags'] if method['flags'] is not None else 0
            temp_ret_left, temp_corners_left = cv2.findChessboardCorners(
                processed_left, chessboard_size, flags=flags)
            temp_ret_right, temp_corners_right = cv2.findChessboardCorners(
                processed_right, chessboard_size, flags=flags)

            # 如果双侧都成功检测到，使用这个方法
            if temp_ret_left and temp_ret_right:
                ret_left, corners_left = temp_ret_left, temp_corners_left
                ret_right, corners_right = temp_ret_right, temp_corners_right
                used_method = method['name']
                break
            # 如果当前方法比之前的方法好，更新结果
            elif (temp_ret_left or temp_ret_right) and not (ret_left or ret_right):
                ret_left, corners_left = temp_ret_left, temp_corners_left
                ret_right, corners_right = temp_ret_right, temp_corners_right
                used_method = method['name']
        ...
        if ret_left:
            print(f"  [成功] 在左侧图像中找到角点: {os.path.basename(fname_left)} (方法: {used_method})")
        ...
        if ret_right:
            print(f"  [成功] 在右侧图像中找到角点: {os.path.basename(fname_right)} (方法: {used_method})")
