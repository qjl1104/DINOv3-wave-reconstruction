...
def strict_tracking(all_detections_per_frame, params):
    """严格的跟踪算法"""
...
def visualize_strict_tracking(frame_idx, img_path, detections, tracks):
    """可视化"""
    ...
    for det in detections:
        cv2.circle(img, (int(det[0]), int(det[1])), 2, (0, 0, 255), -1)
        ...
        if track.last_frame_seen >= frame_idx - 10:
            ...
            if len(points) > 1:
                ...
                for i in range(len(recent) - 1):
                    p1 = tuple(map(int, recent[i]))
                    p2 = tuple(map(int, recent[i + 1]))
                    cv2.line(img, p1, p2, color, thickness)

    cv2.putText(img, f"Frame {frame_idx} | Tracks: {len(tracks)}",
                (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
    cv2.imshow('Strict Tracking', img)
    cv2.waitKey(1)
...
def run_strict_tracking(det_left, det_right, out_left, out_right,
    ...
    # 左侧
    print("\n--- 左侧相机 ---")
    tracks_left = strict_tracking(detections_left, params)

    # 右侧
    print("\n--- 右侧相机 ---")
    tracks_right = strict_tracking(detections_right, params)
